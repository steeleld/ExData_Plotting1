...
# trying png instead of copying output for legend cut-off reasons
png(file="plot4.png", width=480, height=480, units="px",
   res=72, pointsize=12, bg = "white")
par(mfrow=c(2,2), mar=c(4,4,2,1), oma=c(0,0,2,0))

# plot on top left  is the same as plot2.png
...
# plot on top right is a new graph that shows Voltage vs. datetime
plot(plotdata$datetime, plotdata$Voltage, type="n", 
       ylab="Voltage", xlab="datetime")
    # add data as a line graph
    lines(plotdata$datetime, plotdata$Voltage)
    
# plot on bottom left is the same as plot3.png
...
# plot on bottom right is a new line graph showing reactive power vs datetime
